...
util.AddNetworkString( "BricksServerNet_SendGangInbox" )
function BRICKS_SERVER.Func.SendGangInbox( ply )
	...
	net.Start( "BricksServerNet_SendGangInbox" )
		net.WriteTable( inboxTable )
	net.Send( ply )
end

hook.Add( "BRS_PlayerGangInitialize", "BricksServerHooks_BRS_PlayerGangInitialize_GangInbox", function( ply )
	BRICKS_SERVER.Func.SendGangInbox( ply )
end )

util.AddNetworkString( "BricksServerNet_SendGangInboxEntry" )
function BRICKS_SERVER.Func.AddGangInboxEntry( ReceiverSteamID, ReceiverGangID, Type, ReqInfo )
	...
	if( ReceiverSteamID ) then
		...
		if( IsValid( ply ) ) then
			net.Start( "BricksServerNet_SendGangInboxEntry" )
				net.WriteString( key )
...
util.AddNetworkString( "BricksServerNet_RemoveGangInboxEntry" )
function BRICKS_SERVER.Func.DeleteGangInboxEntry( receiverKey, inboxKey )
	...
	if( BRICKS_SERVER_GANGS[receiverKey] ) then
		...
		if( #onlineMembers > 0 ) then
			net.Start( "BricksServerNet_RemoveGangInboxEntry" )
...
util.AddNetworkString( "BricksServerNet_DeleteGangInboxEntry" )
net.Receive( "BricksServerNet_DeleteGangInboxEntry", function( len, ply ) 
