util.AddNetworkString( "BricksServerNet_SetGangTableValue" )
util.AddNetworkString( "BricksServerNet_SetGangTableValues" )
    ...
    if( BRICKS_SERVER_GANGS[gangID] ) then
        ...
        if( #onlineMembers > 0 ) then
            if( table.Count( valuesChanged ) == 1 ) then
                ...
                net.Start( "BricksServerNet_SetGangTableValue" )
                ...
                net.Start( "BricksServerNet_SetGangTableValues" )
                    net.WriteUInt( gangID, 16 )
    ...
    BRICKS_SERVER.Func.FetchGangsDB( function( data )
        ...
        if( BRICKS_SERVER.Func.IsSubModuleEnabled( "gangs", "printers" ) ) then
            BRICKS_SERVER.Func.FetchGangPrinterDB( function( data )
                ...
                hook.Run( "BRS_GangDataLoaded" )
            end )
        end
    end )
end

if( not BRICKS_SERVER.GANGS.LUACFG.UseMySQL or BRICKS_SERVER.INITIALIZE_LOADED ) then
    loadGangs()
else
    hook.Add( "Initialize", "BricksServerHooks_Initialize_Gangs", loadGangs )
end

hook.Add( "BRS_PlayerFullLoad", "BricksServerHooks_BRS_PlayerFullLoad_Gangs", function( ply )	
    ...
    hook.Run( "BRS_PlayerGangInitialize", ply ) 
end )

util.AddNetworkString( "BricksServerNet_SetGangTable" )
function BRICKS_SERVER.Func.SendGangTable( ply, gangID )
    local gangTable = BRICKS_SERVER_GANGS[gangID]

    net.Start( "BricksServerNet_SetGangTable" )
        net.WriteUInt( gangID, 16 )
