util.AddNetworkString( "BricksServerNet_OpenGangMenu" )
function BRICKS_SERVER.Func.OpenGangMenu( ply )
	net.Start( "BricksServerNet_OpenGangMenu" )
	net.Send( ply )
end
...
function BRICKS_SERVER.Func.AddGangExperience( gangID, amount )
...
function BRICKS_SERVER.Func.SetGangLevel( gangID, newLevel )
	...
	hook.Run( "BRS_GangLevelChanged", gangID, finalLevel )
end
...
util.AddNetworkString( "BricksServerNet_CreateGang" )
net.Receive( "BricksServerNet_CreateGang", function( len, ply ) 
	local gangIcon = net.ReadString()
	...
	if( string.len( gangName ) > BRICKS_SERVER.DEVCONFIG.GangNameCharLimit ) then
		DarkRP.notify( ply, 1, 5, BRICKS_SERVER.Func.L( "gangNameLength", BRICKS_SERVER.DEVCONFIG.GangNameCharLimit ) )
		return
	end

	if( string.len( gangIcon ) > BRICKS_SERVER.DEVCONFIG.GangIconCharLimit ) then
		DarkRP.notify( ply, 1, 5, BRICKS_SERVER.Func.L( "gangIconLength", BRICKS_SERVER.DEVCONFIG.GangIconCharLimit ) )
		return
	end
...
util.AddNetworkString( "BricksServerNet_SaveGangRoles" )
net.Receive( "BricksServerNet_SaveGangRoles", function( len, ply ) 
...
util.AddNetworkString( "BricksServerNet_GangSetRank" )
net.Receive( "BricksServerNet_GangSetRank", function( len, ply ) 
...
util.AddNetworkString( "BricksServerNet_SaveGangSettings" )
net.Receive( "BricksServerNet_SaveGangSettings", function( len, ply ) 
	local compressedGangSettings = net.ReadData( len )

	if( not compressedGangSettings ) then return end

	if( not ply:GangHasPermission( "EditSettings" ) ) then return end
	...
	local unCompressedGangSettings = util.Decompress( compressedGangSettings )
	local newGangSettings = util.JSONToTable( unCompressedGangSettings )

	if( not newGangSettings ) then return end

	local nameChanged = false
	if( newGangSettings.gangName and newGangSettings.gangName != gangTable.Name ) then
		nameChanged = true

		if( string.len( newGangSettings.gangName ) > BRICKS_SERVER.DEVCONFIG.GangNameCharLimit ) then
			DarkRP.notify( ply, 1, 5, BRICKS_SERVER.Func.L( "gangNameLength", BRICKS_SERVER.DEVCONFIG.GangNameCharLimit ) )
			return
		end
	end
	...
	if( newGangSettings.gangIcon and newGangSettings.gangIcon != gangTable.Icon ) then
		iconChanged = true

		if( string.len( newGangSettings.gangIcon ) > BRICKS_SERVER.DEVCONFIG.GangIconCharLimit ) then
			DarkRP.notify( ply, 1, 5, BRICKS_SERVER.Func.L( "gangIconLength", BRICKS_SERVER.DEVCONFIG.GangIconCharLimit ) )
			return
		end
	end
	...
	if( nameChanged and iconChanged ) then
		BRICKS_SERVER.Func.UpdateGangTable( ply:GetGangID(), "Name", newGangSettings.gangName, "Icon", newGangSettings.gangIcon )
	elseif( nameChanged ) then
		BRICKS_SERVER.Func.UpdateGangTable( ply:GetGangID(), "Name", newGangSettings.gangName )
	elseif( iconChanged ) then
		BRICKS_SERVER.Func.UpdateGangTable( ply:GetGangID(), "Icon", newGangSettings.gangIcon )
	end

	DarkRP.notify( ply, 1, 5, BRICKS_SERVER.Func.L( "gangSettingsUpdated" ) )
end )

util.AddNetworkString( "BricksServerNet_GangKick" )
net.Receive( "BricksServerNet_GangKick", function( len, ply ) 
	local memberSteamID = net.ReadString()
...
util.AddNetworkString( "BricksServerNet_GangInvite" )
util.AddNetworkString( "BricksServerNet_GangInviteSend" )
net.Receive( "BricksServerNet_GangInvite", function( len, ply ) 
...
util.AddNetworkString( "BricksServerNet_GangInviteAccept" )
net.Receive( "BricksServerNet_GangInviteAccept", function( len, ply ) 
	...
	hook.Run( "BRS_GangMembersChanged", gangID, table.Count( gangMembers ) )
...
util.AddNetworkString( "BricksServerNet_GangDepositMoney" )
net.Receive( "BricksServerNet_GangDepositMoney", function( len, ply ) 
	...
	hook.Run( "BRS_GangBalanceChanged", ply:GetGangID(), math.Clamp( gangBalance+depositAmount, 0, maxBalance ) )
end )

util.AddNetworkString( "BricksServerNet_GangWithdrawMoney" )
net.Receive( "BricksServerNet_GangWithdrawMoney", function( len, ply ) 
...
util.AddNetworkString( "BricksServerNet_GangDisband" )
net.Receive( "BricksServerNet_GangDisband", function( len, ply ) 
	if( not ply:HasGang() ) then return end
...
util.AddNetworkString( "BricksServerNet_GangTransfer" )
net.Receive( "BricksServerNet_GangTransfer", function( len, ply ) 
	local memberSteamID = net.ReadString()
...
util.AddNetworkString( "BricksServerNet_GangLeave" )
net.Receive( "BricksServerNet_GangLeave", function( len, ply ) 
	if( not ply:HasGang() ) then return end
...
util.AddNetworkString( "BricksServerNet_GangNetworkMessage" )
function BRICKS_SERVER.Func.AddGangChatMessage( gangID, message, memberSteamID )
	...
	net.Start( "BricksServerNet_GangNetworkMessage" )
...
util.AddNetworkString( "BricksServerNet_GangSendMessage" )
net.Receive( "BricksServerNet_GangSendMessage", function( len, ply ) 
...
hook.Add( "EntityTakeDamage", "BricksServerHooks_EntityTakeDamage_GangFF", function( target, dmginfo )
	if( not BRICKS_SERVER.CONFIG.GANGS["Gang Friendly Fire"] and target:IsPlayer() and IsValid( dmginfo:GetAttacker() ) and dmginfo:GetAttacker():IsPlayer() and target:HasGang() and target:GetGangID() == dmginfo:GetAttacker():GetGangID() ) then
		local shouldTakeDamage = hook.Run( "BRS_CanTakeDamageFF", target, dmginfo:GetAttacker() )
...
util.AddNetworkString( "BricksServerNet_RequestPlyGangInfo" )
util.AddNetworkString( "BricksServerNet_SendPlyGangInfo" )
net.Receive( "BricksServerNet_RequestPlyGangInfo", function( len, ply )
	...
	if( gangTable ) then
		...
		net.Start( "BricksServerNet_SendPlyGangInfo" )
