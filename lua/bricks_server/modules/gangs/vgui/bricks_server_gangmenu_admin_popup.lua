...
function PANEL:FillPanel( panelWide, panelTall )
    ...
    hook.Add( "BRS_RefreshGangAdminData", "BricksServerHooks_BRS_RefreshGangAdminData_" .. tostring( self.popout ), function( gangID, gangTable )
        if( IsValid( self ) and IsValid( self.popout ) ) then
            self:RefreshPanel( gangID, gangTable )
        else
            hook.Remove( "BRS_RefreshGangAdminData", "BricksServerHooks_BRS_RefreshGangAdminData_" .. tostring( self.popout ) )
        end
    end )
end
...
function PANEL:RefreshPanel( gangID, gangTable )
    ...
    local iconMat
    BRICKS_SERVER.Func.GetImage( gangTable.Icon or "question.png", function( mat ) 
        iconMat = mat 
    end )
    infoPanel.Paint = function( self2, w, h )
        local avatarSize = (gangTable.Icon and h-6) or 32
        ...
        if( iconMat ) then
            ...
            BRICKS_SERVER.Func.DrawCircle( h/2, h/2, avatarSize/2, avatarSize/2 )
            ...
            surface.DrawTexturedRect( (h/2)-(avatarSize/2), (h/2)-(avatarSize/2), avatarSize, avatarSize )
        ...
        actionButton.DoClick = function()
            ...
            elseif( v.ServerFunc ) then
                GetNextQuery( 0, v.ReqInfo, {}, function( reqInfo )
                    net.Start( "BricksServerNet_AdminGangCMD" )
                        net.WriteUInt( k, 8 )
