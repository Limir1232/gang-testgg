...
function PANEL:FillPanel( gangTable )
    function self.RefreshPanel()
        ...
        for k, v in pairs( BRICKS_SERVER.CONFIG.GANGS.Territories or {} ) do
            ...
            local iconMat
            BRICKS_SERVER.Func.GetImage( claimedGangTable.Icon or "question.png", function( mat ) 
                iconMat = mat 
            end )
            avatarBack.Paint = function( self2, w, h )
                local avatarBackSize = 70
                local avatarSize = (claimed and h-6) or 32
                ...
                draw.SimpleText( BRICKS_SERVER.Func.L( "gangID", (territoryTable.GangID or 0) ), "BRICKS_SERVER_Font17", textStartPos, h/2-2, BRICKS_SERVER.Func.GetTheme( 6 ), 0, 0 )
    ...
    hook.Add( "BRS_RefreshGangTerritories", self, function()
        if( IsValid( self ) ) then
            self.RefreshPanel()
        else
            hook.Remove( "BRS_RefreshGangTerritories", self )
        end
    end )
end
