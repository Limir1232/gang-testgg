...
function ENT:CreateVGUI()
	...
	if( not IsValid( self.printerVGUI )  ) then
		local outerMargin, innerSpacing = 15, 10
		...
		self.printerVGUI.CreateCursor = function( self2 )
			...
			self.printerVGUI.Cursor:SetSize( 16, 16 )
		...
		gangPanel.Paint = function( self2, w, h )
			...
			draw.SimpleText( ((gangTable or {}).Name or "Unknown"), "BRICKS_SERVER_Font33", iconBackSpacing+iconBackSize+20, h/2+2, BRICKS_SERVER.Func.GetTheme( 6 ), 0, TEXT_ALIGN_BOTTOM )
			draw.SimpleText( (printerConfigTable.Name or "Printer 0"), "BRICKS_SERVER_Font24", iconBackSpacing+iconBackSize+20, h/2-2, greyTextCol, 0, 0 )
		...
		healthPanel.Paint = function( self2, w, h )
			...
			draw.SimpleText( "HEALTH", "BRICKS_SERVER_Font24", 23, 15, Color( 255, 255, 255, 25 ), 0, 0 )

			local barH, barSideMargin = 13, 23
			draw.RoundedBox( 8, barSideMargin, 45, w-(2*barSideMargin), barH, BRICKS_SERVER.Func.GetTheme( 3 ) )
			...
			draw.RoundedBox( 8, barSideMargin, 45, (w-(2*barSideMargin))*healthPercent, barH, healthColor )
		end
		...
		local radius, arcWidth, mainArcWidth = 80, 2, 6
		...
		for k, v in ipairs( ProgressBars ) do
			...
			progressPanel.Paint = function( self2, w, h )
				...
				draw.SimpleText( v[2](), "BRICKS_SERVER_Font24", w/2, h/2, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
				draw.SimpleText( v[1], "BRICKS_SERVER_Font24", w/2, h/2+radius+15, Color( 255, 255, 255, 25 ), TEXT_ALIGN_CENTER, 0 )
			end
		end
		...
		graphPanel:SetTall( 333 )
		graphPanel.Paint = function( self2, w, h )
			draw.RoundedBox( 8, 0, 0, w, h, BRICKS_SERVER.Func.GetTheme( 0 ) )

			draw.SimpleText( "INCOME", "BRICKS_SERVER_Font24", 23, 21, Color( 255, 255, 255, 25 ), 0, 0 )
		end
		...
		graphLinesPanel.Paint = function( self2, w, h )
			...
			for i = 1, lineCount do
				...
				draw.SimpleText( ((lineValue >= 100 and displayValue .. "k") or displayValue), "BRICKS_SERVER_Font17", lineDistance/2, h-(i*(h/lineCount))-1, BRICKS_SERVER.Func.GetTheme( 5 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			end
		...
		local bottomLeftPanel = vgui.Create( "DPanel", self.printerVGUI )
		...
		bottomLeftPanel.Paint = function( self2, w, h )
			...
			draw.SimpleText( "HISTORY", "BRICKS_SERVER_Font24", 23, 21, Color( 255, 255, 255, 25 ), 0, 0 )
		end

		local bottomRightPanel = vgui.Create( "DPanel", self.printerVGUI )
		...
		toggleButton.Paint = function( self2, w, h )
			...
			draw.SimpleText( "ON", "BRICKS_SERVER_Font20", w/4, h/2, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			draw.SimpleText( "OFF", "BRICKS_SERVER_Font20", w/4*3, h/2, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end
		toggleButton.DoClick = function()
			net.Start( "BricksServerNet_GangPrinterToggle" )
		...
		withdrawButton.Paint = function( self2, w, h )
			...
			draw.SimpleText( "WITHDRAW MONEY", "BRICKS_SERVER_Font20", w/2, h/2, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end
		withdrawButton.DoClick = function()
			net.Start( "BricksServerNet_GangPrinterWithdraw" )
				net.WriteEntity( self )
			net.SendToServer()
		end
		...
		menuPanel.Paint = function( self2, w, h )
			...
			draw.SimpleText( "MENU", "BRICKS_SERVER_Font24", 23, 21, Color( 255, 255, 255, 25 ), 0, 0 )
		end
		...
		for k, v in ipairs( pages ) do
			...
			button.Paint = function( self2, w, h ) 
				...
				draw.SimpleText( v.Name, "BRICKS_SERVER_Font20", w/2, h/2, Color( 255, 255, 255, 50 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			end
			button.DoClick = function()
				...
				surface.SetFont( "BRICKS_SERVER_Font40" )
				local textX, textY = surface.GetTextSize( v.Name )
				...
				self.printerVGUI.printerPage.Paint = function( self2, w, h )
					...
					draw.SimpleText( v.Name, "BRICKS_SERVER_Font40", 24+51+25, 12+(75/2)-2, BRICKS_SERVER.Func.GetTheme( 6 ), 0, TEXT_ALIGN_CENTER )
				end
...
function ENT:Draw()
	...
	vgui.Start3D2D( Pos+(Ang:Up() * 30.55)-(Ang:Forward()*11.35)-(Ang:Right()*45.03), Ang, 0.03 )
...
function ENT:CreateRequestTimer()
	net.Start( "BricksServerNet_GangPrinterIncomeTrackRequest" )
		net.WriteEntity( self )
	net.SendToServer()
...
net.Receive( "BricksServerNet_GangPrinterIncomeTrackSend", function()
